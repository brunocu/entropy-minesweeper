...
describe('QuickXplain matches the paper (arXiv:2001.01835 Alg. 1, Ex. 6)', () => {
  ...
  it('evaluates the predicate on exactly the sets the worked example evaluates it on', () => {
    ...
    // background since the parent's own test - so the answer is already known. `quickXplain` has
    // no C parameter and re-asks. Every such re-ask is therefore a verbatim repeat of an earlier
    // query (or `p({})` at the root, where B = {} was never tested at all), never new work: in the
    // solver those land on the subset-verdict cache. Dropping them recovers the paper's table.
