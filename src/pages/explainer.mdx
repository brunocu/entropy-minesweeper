---
layout: ../layouts/Explainer.astro
title: What does any of this mean? — Entropy Minesweeper
---

import { RevealDemo } from '../components/RevealDemo.tsx'
import { computeFigureValues } from '../explainer/figures.ts'

{/* Every number and cell label quoted below comes from here, so the article is a solve rather than
    a transcription of one. Runs once, at build time. */}
export const figures = computeFigureValues()

export const illustration = (name) => `${import.meta.env.BASE_URL}assets/explainer/${name}.svg`
...
<figure class="board-figure">
  <img
    src={illustration('intro-trivial-board')}
...
<figure class="board-figure">
  <img
    src={illustration('intro-open-board')}
  ...
  <figcaption>
    {figures['mine-count']} mines and five covered cells, and not one clue here can be resolved
...
<figure class="figure-wide">
  <div class="root-board-panel">
    <img
      src={illustration('worlds-tree-board')}
  ...
  <div class="figure-scroll">
    <img
      src={illustration('worlds-tree-probability')}
  ...
  <figcaption>
    {figures['mine-count']} mines, {figures['unknown-count']} cells still
    unknown ({figures['unknown-labels']}), and the branching search over them,
    each inconsistent branch cut at the depth the clues rule it out.<br />
    The highlighted branches are the consistent worlds where A2 is a mine. Their weights sum to
    {figures['focus-probability']},
    because A2's mine probability is just the count of consistent branches where A2 is a mine over the
    total count of consistent branches: $2/4$.<br />
...
<span class="term-safe">deep blue</span> at $P = 0$, a pale neutral at
$P = 0.5$, <span class="term-mine">red</span> at
$P = 1$.
...
<figure class="figure-wide">
  ...
  <div class="figure-scroll">
    <img
      src={illustration('worlds-tree-eig')}
  ...
  <figcaption>
    ...
    {figures['world-count']} consistent worlds,
...
{figures['open-board-certain-clues']} together force
{figures['open-board-certain-cell']} to be a mine, and dropping any one of the three leaves an
arrangement where it isn't.

Hovering a certain cell in the game highlights what survives that trimming: the revealed
<span class="term-clue">clues</span> the argument rests on, and any unrevealed
<span class="term-premise">premise</span> cells whose own forced status it leans on.

<figure class="board-figure">
  <img
    src={illustration('certainty-board')}
    alt="A toy board where C1 is certainly safe, with its two clue cells highlighted in aqua and its premise cell in green."
  />

  <figcaption>
    A different board state, with one certainly-safe cell. Read it in two steps.<br />
    The `1` at E1 has exactly one unrevealed neighbour, so that neighbour must be
    the mine, the premise.<br />
    The other `1` at D2 sees only that premise and the ringed cell, and
    its single mine is already spoken for, so the next cell is safe.<br />
    In full: {figures['certainty-focus']} is certainly safe because of the clues at
    <span class="term-clue">{figures['certainty-clues']}</span>, given that
    <span class="term-premise">{figures['certainty-premises']}</span> must be a mine.
  </figcaption>
</figure>
...
<figure>
  <div class="figure-scroll">
    <img
      src={illustration('uncertainty-chart')}
