...
import { BoardRenderer, type RenderBoard, type RenderCell } from '../canvas/boardRenderer.ts'
...
import { WORLDS_TREE_BOARD, WORLDS_TREE_FOCUS_CELL } from '../explainer/fixtures.ts'
import { simulateReveal, worldCount, type SimulatedReveal } from '../explainer/predictedVsRealized.ts'
...
export function RevealDemo(props: RevealDemoProps): JSX.Element {
  ...
  /**
   * Rings the one clickable cell, drawn onto the canvas after the board so it layers over the fill.
   * `BoardRenderer` has no notion of "the cell this widget is about", and teaching it one would put
   * an explainer-only concept into the live game's renderer.
   */
    ...
    <div class="demo" id="predicted-vs-realized">
      <div class="demo-board">
        <canvas
          class="demo-canvas"
          ref={canvas}
          // Only the focus cell is live: revealing another would leave the predicted number on
          // screen describing a click that never happened. The cursor says so.
          onMouseMove={(event) => {
