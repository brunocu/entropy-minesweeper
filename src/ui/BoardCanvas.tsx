...
import { BoardRenderer, type HighlightRole, type RenderBoard } from '../render/boardRenderer.ts'
import { cellSolverValues } from '../render/cellSolverValues.ts'
import { type GridCoord, pixelToCell } from '../render/hitTest.ts'
...
interface BoardCanvasProps {
  ...
  readonly setHoveredCell: (cell: GridCoord | null) => void
  readonly onReveal: (cell: GridCoord) => void
  readonly onToggleFlag: (cell: GridCoord) => void
}
...
export function BoardCanvas(props: BoardCanvasProps) {
  ...
  /**
   * The hovered cell's highlight, or null when it has none. Derived rather than assigned, so a
   * pointer moving between two cells that neither explain anything produces `null` both times and
   * the render effect below - which reads this - is never notified.
   */
  ...
  function cellAt(event: MouseEvent): GridCoord | null {
