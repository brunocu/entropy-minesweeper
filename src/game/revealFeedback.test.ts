...
import { solve, type SolverBoard, type SolverCell } from '../solver/frontierSolver.ts'
...
describe('frontier EIG readout (6.1)', () => {
...
describe('predicted-vs-realized reveal feedback (6.2/6.3)', () => {
  it('returns predicted EIG and revealed information for a frontier reveal', () => {
    ...
    const postReveal = solve(postBoard, new Map()).result
  ...
  it('reports revealed information for a whole cascade, not just the clicked cell', () => {
    ...
    // pre-reveal solve has no frontier entry for it at all, so the old single-cell
    // realizedSurprisal approach couldn't report anything for this reveal (computeRevealFeedback
    // used to return null here). Clicking it cascades: 0 -> 0 -> '1' (stops), which pins index 3
