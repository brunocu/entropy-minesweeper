// Traces 1:1 to the scenarios in openspec/specs/information-visualization/spec.md
//
...
// in the DOM) were verified visually in-browser during implementation - this sandbox has no
// headless browser to re-run those automatically.
...
import { MINE_POLE_COLOR, NEUTRAL_MIDPOINT_COLOR, SAFE_POLE_COLOR, probabilityColor } from '../scale/probabilityColor.ts'
import { makeBoard } from './support/makeBoard.ts'

describe('information-visualization spec scenarios', () => {
  ...
  it('Predicted and realized values are both available for a frontier reveal', () => {
    ...
    const postReveal = solve(decompose(makeBoard(['11', '1?'], 1)), new Map()).result
  ...
  it('Non-frontier reveals show no predicted EIG, but still show revealed information', () => {
    const board = makeBoard(['?', '1', '?', '1', '?', '?', '?'], 2)
    const preReveal = solve(decompose(board), new Map()).result
    const postReveal = solve(decompose(makeBoard(['?', '1', '?', '1', '?', '0', '?'], 2)), new Map()).result
