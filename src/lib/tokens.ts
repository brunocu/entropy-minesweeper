// The site's colors, in one place. The canvas renderers import these values directly (they need JS
// strings at draw time), and `renderTokensCss` derives the pages' `:root` custom properties.
//
// Names are semantic, never literal: `safe`/`mine`, not `blue`/`red`. The explainer's `.term-safe`
// span and the board's certainly-safe cell are the same token because they mean the same thing.
export const tokens = {
