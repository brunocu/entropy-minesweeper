// Traces 1:1 to the scenarios in openspec/specs/frontier-solver/spec.md
...
describe('frontier-solver spec scenarios', () => {
  ...
  it('Frontier deduction shifts non-frontier probability', () => {
    const before = solve(decompose(makeBoard(['?', '1', '?', '1', '?', '?', '?'], 2)), new Map()).result
    const after = solve(decompose(makeBoard(['?', '1', '?', '1', '?', '?', '?'], 3)), new Map()).result
