...
import { key, type Coord, type ForcedCells, type FrontierCellResult, type SolveResult } from './types.ts'

/**
 * Everything the per-certain-cell search needs from the component it is explaining, built once per
 * component per `computeExplanations` call rather than once per certain cell.
 *
 * `constraints` is this component's clues, not the board's, so clues that must not appear in an
 * explanation are not in scope to appear. Every constraint's cells lie wholly inside one component,
 * so a BFS over the "clues share a frontier cell" relation can never leave it.
 */
export interface ComponentIndex {
  /** This component's clues. */
  readonly constraints: readonly RawConstraint[]
  /** cellKey -> the clues referencing it. */
  ...
  readonly flagGivens: ForcedCells
...
/**
 * The cache is a parameter so a test can share one across calls, but it belongs to the index: one
 * reached through the index cannot be handed a subset from another component, which is what keeps
 * `flagGivens` constant within it and so hoistable into `givensPrefix`.
 */
export function buildComponentIndex(
  constraints: readonly RawConstraint[],
  flagGivens: ForcedCells,
...
/** The clues sharing a frontier cell with `clue`, expanded from `cluesByCell` on demand. */
...
/**
 * BFS layers of constraints outward from a frontier cell: layer 1 is every constraint touching the
 * cell directly, layer 2 shares a frontier cell with layer 1, and so on. Same-distance constraints
 * land in one layer, sorted by key for determinism, never split by a per-cell tie-break.
 *
 * One layer per pull rather than walking the component to exhaustion: `growSufficientSet` stops as
 * soon as the accumulated set resolves the cell - typically at layer 1 or 2 - and abandoning the
 * generator there leaves the rest of the component unexpanded.
 */
...
// The oracle's work depends only on the candidate subset and `flagGivens`, never on which cell is
// asking - so each subset caches its *full* forced-status result for any later query to read.

/**
 * One subset's cached verdicts, in two tiers so the exponential half stays lazy: a query Tier-0
 * already answers must not pay for enumeration just to fill the cache for other cells.
 */
export interface SubsetCacheEntry {
  /** Tier-0 fixpoint over the subset, seeded with `flagGivens`. */
  readonly tier0: ForcedCells
  /** Backtracking-derived verdict, unseeded by flags; computed on first demand only. */
  enumerated?: ForcedCells
}
 ...
 * `computeExplanations` call `buildConstraints` emits at most one constraint per numbered cell, so
 * a clue's id pins down its `key`, `cells` and `requiredMines` completely.
 */
...
/**
 * The subset's Tier-0 verdict, memoized in the component's cache. `growTrimCallCount` increments
 * here and only here: a miss is exactly one genuine (re)computation of a search step.
 */
...
export function verdictHas(verdict: ForcedCells, xKey: string, targetValue: 0 | 1): boolean {
...
/**
 * Trim phase: QuickXplain's divide-and-conquer minimization - procedure QX' of Alg. 1 in Rodler,
 * "Understanding the QuickXPlain Algorithm" (arXiv:2001.01835), itself Junker 2004. Adapted to a
 * "sufficiency" polarity: `p(S)` = "S forces the cell", the monotone property the proof requires,
 * since a superset of a sufficient set can only gain information.
 *
 * Returns an irreducible subset of `candidates` that, with `background`, still satisfies `p`.
 * Callers must pass a `candidates` set already known sufficient - `growSufficientSet` returning
 * non-`null` is that guarantee - since the paper places the `p(A u B) = 0` check in QX, above QX',
 * and QX' has no way to signal it.
 *
 * `p` is a parameter rather than a closed-over `resolvesTo` for the reason the paper gives for its
 * own formulation (Sec. 1, criterion (i)): the predicate is a black box taking a subset and
 * returning a bit. That keeps the algorithm free of anything minesweeper-specific.
 */
...
export function extractPremiseKeys(
  ...
  // A flag-seeded fact is a premise of this explanation only when it is a neighbor referenced by
  // one of S's own constraints - otherwise seeding surfaces an unrelated forced cell from
...
export function computeExplanations(
  ...
  // Reuse a cached enumeration/explanations set when the component's signature (constraints plus
  // the flagged subset of its cells) matches. The signature is built here rather than in the shared
  // decomposition because it is the one part that depends on flags: `solve` signs the same
  // components with the empty flagged set, and both must keep doing so.
  ...
  for (const { cells, relevantConstraints } of componentSlices) {
    ...
    if (!explanationsForComponent) {
      // Clue index and subset-verdict cache, built once and shared by every certain cell here.
      // Subsets from different components are disjoint, so a per-component cache loses no hit.
